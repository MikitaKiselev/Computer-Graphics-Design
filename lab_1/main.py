...
def rgbChanges(q):
    if whichColors.get() == 'RGB' or q == 1000:
        h, l, s = colorsys.rgb_to_hls(slidersRGB[0].get() / 255, slidersRGB[1].get() / 255, slidersRGB[2].get() / 255)
...
def hslChanges(q):
    if whichColors.get() == 'HSL':
        r, g, b = colorsys.hls_to_rgb(slidersHLS[0].get() / 360, slidersHLS[1].get() / 100, slidersHLS[2].get() / 100)
...
for i in range(3):
    slidersRGB[i].grid(column=0, row=i + 2)
    slidersHLS[i].grid(column=2, row=i + 2)
    slidersLAB[i].grid(column=4, row=i + 2)
...
for j, txt in enumerate(txts):
    for i in range(3):
        ...
        txt[i].insert(0, '0')
        txt[i].grid(column=(j * 2) + 1, row=i + 2)
...
for i, btn in enumerate(btns):
    ...
    newbtn.grid(row=1, column=i * 2, columnspan=2)
    newbtn.select()
whichColors.set("RGB")
...
Button(text='Choose color', command=colorPalette, width=5, height=5).grid(column=2, row=6, columnspan= 2, sticky="nsew")
